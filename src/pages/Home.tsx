...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      const navigationState = location.state as {
        state?: { // The actual state object is nested
          checkIn?: Date;
          checkOut?: Date;
          guests?: string;
          searchText?: string;
      ...
      if (state) { // Ensure the nested state object exists
        ...
        if (state.selectedDates?.checkIn) {
          ...
          localStorage.setItem("checkInDate", state.selectedDates.checkIn.toISOString());
        }
        ...
        if (state.selectedDates?.checkOut) {
          ...
          localStorage.setItem("checkOutDate", state.selectedDates.checkOut.toISOString());
        }
        ...
        if (state.searchText) {
          console.log("Received search text from suggestions page:", state.searchText);
          console.log("Received suggestion from suggestions page:", state.suggestion);
        }
      }
  ...
  useEffect(() => {
    ...
    const ready = !!checkIn && !!checkOut && guests !== "" && !isNaN(guestsNum) && guestsNum > 0;
  ...
  const handleDatesSelected = useCallback((dates: { checkIn: Date | null; checkOut: Date | null }) => {
                  ...
                  <IonCard className="search-card">
                    <IonCardContent className="search-card-content">
                      ...
                      <IonButton
                        ...
                        fill="clear"
                        color="medium"
                        onClick={() => ionRouter.push("/searchSuggestions", "forward")}
                      ...
                      >
                        <IonIcon icon={searchOutline} slot="start" style={{ marginRight: '8px' }} />
                        <IonLabel style={{ flexGrow: 1, color: 'var(--ion-color-medium)'}}>
                          Where are you going?
                        </IonLabel>
                      </IonButton>
                      ...
                      <div className="date-guest-row">
                        ...
                        <IonButton
                          ...
                          onClick={() => {
                            ...
                            ionRouter.push("/calendarMobile", { state: { onDatesSelect: handleDatesSelected } });
                          }}
                      ...
                      {(checkIn || checkOut || guests) && (
                        ...
                        >
                          ...
                          {guests && (
                            <>
                              <span style={{ whiteSpace: 'nowrap' }}>{checkIn && checkOut ? " , " : ""}<strong>{guests}</strong> guest{parseInt(guests) > 1 ? "s" : ""}</span>
                            </>
                          )}
                        </div>
                      )}
